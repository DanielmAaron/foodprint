...
const initialize = () => {
  ...
  $('#getProduceHarvestCountBtn').click((e) => {
    e.preventDefault();
  ...
  function handle_web3_undefined_error() {
    ...
    const message_description = 'Please install MetaMask to access the Ethereum Web3 injected API from your Web browser.';
  ...
  async function addHarvestToBlockchain() {
    ...
    const photoHash = sha256(harvest_photohash);
    ...
    try {
      ...
      const message_description = `Transaction submitted to Blockchain for processing (Upload Harvest Entry 
  ...
  async function verifyHarvestEntry(data) {
    ...
    try {
      ...
      const contractIsSet = result[4].toNumber();
      ...
      if (contractIsSet > 0) {
        const contract_harvest_id = result[0];
        const contract_harvestEntry_supplierproduceID = result[1];
        const contract_harvestEntry_harvestTimeStamp = result[2];
        const contract_harvestEntry_BlockNumber = result[3];
        // let displayDate = new Date(contractSubmissionBlocktime * 1000).toLocaleString();

        const message_description = `Harvest Entry for ${contract_harvestEntry_supplierproduceID} with Harvest ID ${contract_harvest_id} is <b>valid</b>. Uploaded to FoodPrint Produce (Blockchain) on: ${contract_harvestEntry_harvestTimeStamp}.`
      ...
      const message_description = `Harvest Entry with Harvest ID ${harvest_logid} is <b>invalid</b>: not found in the FoodPrint Harvest Logbook (Blockchain).`;
  ...
  async function retrieveHarvestEntrySubmitAddress() {
    try {
      ...
      if (result === '0x0000000000000000000000000000000000000000') {
        const message_description = `Harvest Entry with Harvest ID ${harvest_logid} is <b>invalid</b>: no corresponding submitter address found in the FoodPrint Produce (Blockchain).`;

        // TODO - trigger notification
        return console.log(message_description);
      }
      const message_description = `Harvest Entry with Harvest ID ${harvest_logid} is <b>valid</b>. Uploaded to FoodPrint Produce (Blockchain) by address : ${result}.`;
      // TODO - trigger notification
      return console.log(message_description);
    } catch (err) {
      console.log('Error: ', err);
  ...
  async function getProduceHarvestCount() {
    ...
    try {
      ...
      const harvestSubmissionsCount = result.toNumber(); // Output from the contract function call
      console.log(`getHarvestSubmissionsCount: ${harvestSubmissionsCount}`);
      const message_description = `Number of Harvest Entries: + ${harvestSubmissionsCount}`;
      // TODO - trigger notification
      return console.log(message_description);
    } catch (err) {
      console.log('Error: ', err);
  ...
  async function addStorageToBlockchain() {
    ...
    const storageDetail = `{storageDescription:${storage_Description}, storageTableName:${storage_tablename}, 
    storageUser:${storage_user}, storageQuantity:${storage_quantity_combined}}`;
    const otherID = `{supplierproduceID:${supplierproduce}, marketID:${market_Shortcode}}`;
    ...
    try {
      ...
      const message_description = `Transaction submitted to Blockchain for processing (Upload Storage Entry from ${supplierproduce} with Harvest ID ${harvest_logid} and Storage ID ${storage_logid}). Check your Metamask for transaction update.`;
      // TODO - trigger notification
      console.log(message_description);
      return;
    } catch (err) {
  ...
  async function verifyStorageEntry(data) {
    ...
    try {
      ...
      // Output from the contract function call
      console.log(`result: ${result}`);
      const contractIsSet = result[5].toNumber();
      console.log(`contractIsSet: ${contractIsSet}`);
      console.log(`result[0]: ${result[0]}`);
      console.log(`(contractIsSet > 0): ${contractIsSet > 0}`);
      ...
      if (contractIsSet > 0) {
        const contract_storage_id = result[0];
        const contract_harvest_id = result[1];
        const contract_otherID = result[2];
        const contract_storageDetail = result[3];
        const contract_storageEntry_BlockNumber = result[4];

        const message_description = `Storage Entry with Storage ID ${contract_storage_id} and Harvest ID ${contract_harvest_id} is <b>valid</b>. This corresponds to ${contract_otherID}. Additional details from 
      ...
      const message_description = `Storage Entry with Storage ID ${contract_storage_id} is <b>invalid</b>: not found in the FoodPrint Storage Logbook (Blockchain).`;
