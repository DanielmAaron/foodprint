...
App = {
  ...
  initContract() {
    $.getJSON('contracts/TheProduct.json', (TheProductArtifact) => {
  ...
  render() {
    let theProductInstance;
    const loader = $('#loader');
    const content = $('#content');

    const loaderStorage = $('#loaderStorage');
    ...
    App.contracts.TheProduct.deployed().then((instance) => {
      theProductInstance = instance;
      return theProductInstance.noHarvests();
    }).then((harvestCount) => {
      const tableHarvest = $('#tableHarvest');
      tableHarvest.empty();

      const inputSupplierStorageSelect = $('#inputSupplierStorage');
      inputSupplierStorageSelect.empty();

      let optionValue = 'Choose harvest entry...';
      let optionId = 'Choose harvest entry...';
      // inputSupplierStorageSelect.attr('placeholder','Choose harvest entry...');
      let inputSupplierStorageOption = `<option value='${optionId}' selected disabled >${optionValue}</ option>`;
      inputSupplierStorageSelect.append(inputSupplierStorageOption);

      for (let i = 0; i <= harvestCount; i++) {
        theProductInstance.harvestProduceArray(i).then((harvest) => {
          console.log(harvest);
          const harvestSupplier = harvest[1];
          const harvestProduct = harvest[3];
          const harvestPhoto = harvest[4];
          const harvestTime = harvest[5];
          const harvestDataCaptureTime = harvest[6];

          // Render Harvest entries
          const harvestEntry = `<tr><td>${harvestSupplier}</td><td>${harvestProduct}</td><td>${
    ...
    App.contracts.TheProduct.deployed().then((instance) => {
      ...
      for (let i = 0; i <= storageCount; i++) {
        theProductInstance.storageProduceArray(i).then((storage) => {
          console.log(storage);
          const storageSupplierProductDate = storage[1];
          const storageQuantity = storage[3];
          const storageUoM = storage[4];
          const storageTime = storage[5];
          const storageDataCaptureTime = storage[6];

          // Render Storage entries
          const storageEntry = `<tr><td>${storageSupplierProductDate}</td><td>${storageQuantity}</td><td>${
  ...
  displayWeeklyView() {
    ...
    let theProductInstance;
    const loaderWeekly = $('#loaderWeekly');
    const contentWeekly = $('#contentWeekly');
    ...
    App.contracts.TheProduct.deployed().then((instance) => {
      ...
      const cardDeckWeekly = $('#cardDeckWeekly');
      cardDeckWeekly.empty();

      for (let i = 0; i <= storageCount; i++) {
        (function (j) {
          theProductInstance.storageProduceArray(j).then((storage) => {
            // console.log(storage);
            const storageSupplierProductDate = storage[1];
            ...
            const supplierProductDateArray = storageSupplierProductDate.split('_');
            const cardTitle = supplierProductDateArray[1]; // produce
            const cardText = supplierProductDateArray[0]; // supplier
            if (typeof cardTitle !== 'undefined') {
              const cardUpdateTime = storageDataCaptureTime;
              const QRCodeSupplierProduce = (`${cardText}_${cardTitle}`).replace(/\s/g, '');
              const QRCodeID = `QRCode_${j}_${QRCodeSupplierProduce}`;

              const weeklyEntry = `${'<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12 ">\n'
                            + '          <div class="service">\n'
              ...
              const QRCodeURL = `${baseURL}/scan/${QRCodeSupplierProduce}`; // http://localhost:3000/scanresult/OranjezichtCityFarm_Apples
  ...
  registerHarvest() {
    const inputSupplier = $('#inputSupplier').val();
    const inputProduct = $('#inputProduct').val();
    const inputPhoto = $('#inputPhoto').val();
    // var inputHarvestTime = $('#inputHarvestTime').data("datetimepicker").date();
    const inputHarvestTime = new Date($('#inputHarvestTime').val());
    // var inputDataTime = $('#inputDataTime').data("datetimepicker").date();
    const inputDataTime = new Date($('#inputDataTime').val());

    const momentHarvestTime = moment(inputHarvestTime).format('YYYY-MM-DD HH:mm');
    const momentInputDataTime = moment(inputDataTime).format('YYYY-MM-DD HH:mm');
    ...
    App.contracts.TheProduct.deployed().then((instance) => {
      ...
      const harvestSupplierProduce = (`${inputSupplier}_${inputProduct}`).replace(/\s/g, '');
  ...
  registerStorage() {
    const inputSupplierStorage = $('#inputSupplierStorage').val();
    const inputQuantityStorage = $('#inputQuantityStorage').val();
    const inputUoMStorage = $('#inputUoMStorage').val();
    // var inputStorageTime = $('#inputStorageTime').data("datetimepicker").date();
    const inputStorageTime = new Date($('#inputStorageTime').val());
    // var inputDataTimeStorage = $('#inputDataTimeStorage').data("datetimepicker").date();
    const inputDataTimeStorage = new Date($('#inputDataTimeStorage').val());

    const momentInputStorageTime = moment(inputStorageTime).format('YYYY-MM-DD HH:mm');
    const momentInputDataTimeStorage = moment(inputDataTimeStorage).format('YYYY-MM-DD HH:mm');
    ...
    App.contracts.TheProduct.deployed().then((instance) => {
      ...
      const supplierProductDateArray = inputSupplierStorage.split('_');
      const supplierName = supplierProductDateArray[0]; // supplier
      const supplierProduce = supplierProductDateArray[1]; // produce
      const storageSupplierProduce = (`${supplierName}_${supplierProduce}`).replace(/\s/g, '');
...
function TriggerAlertOpen(parentDivID, alertDivID, alertMessage) {
  ...
  const divNotificationHtml = `<div id=${alertDivID} class="alert alert-success fade in show"><button type="button" class="close close-alert" data-dismiss="alert" aria-hidden="true">×</button><strong>${alertMessage}</strong></div>`;
...
$('#numHarvestButton').click((e) => {
  ...
  App.contracts.TheProduct.deployed().then((instance) => {
    ...
    const divNumHarvestHtml = `<div class="alert alert-success fade in show"><button type="button" class="close close-alert" data-dismiss="alert" aria-hidden="true">×</button><strong>Number of Harvest entries </strong>${harvestsCount.toString()}</div>`;
...
$('#numStorageButton').click((e) => {
  ...
  App.contracts.TheProduct.deployed().then((instance) => {
    ...
    const divNumStorageHtml = `<div class="alert alert-success fade in show"><button type="button" class="close close-alert" data-dismiss="alert" aria-hidden="true">×</button><strong>Number of Storage entries </strong>${storageCount.toString()}</div>`;
