...
router.get('/',
  ...
  (req, res, next) => {
    if (req.user.role === ROLES.Admin || req.user.role === ROLES.Superuser) {
      connection.execute('SELECT * FROM foodprint_storage ORDER BY pk desc', (err, rows) => {
        if (err) {
          ...
          connection.execute('SELECT harvest_logid, supplierproduce, harvest_quantity, harvest_unitofmeasure, harvest_TimeStamp FROM foodprint_harvest ORDER BY pk desc', (err, harvest_rows) => {
            if (err) {
...
router.post('/save', [
  ...
  check('viewmodal_harvest_suppliershortcode', 'Harvest Supplier Shortcode is not valid').not().isEmpty().trim()
    .escape(),
  check('viewmodal_supplierproduce', ' Supplier Produce value is not valid').not().isEmpty().trim()
    .escape(),
  check('viewmodal_market_Shortcode', 'Market Shortcode value is not valid').not().isEmpty().trim()
    .escape(),
  check('viewmodal_market_Name', 'Market Name value is not valid').not().isEmpty().trim()
    .escape(),
  check('viewmodal_market_Address', 'Market Address value is not valid').not().isEmpty(),
  check('viewmodal_harvest_logidSelect', 'Harvest ID value is not valid').not().isEmpty().trim()
    .escape(),
  check('viewmodal_market_quantity', 'Storage Quantity value is not valid').not().isEmpty().trim()
    .escape(),
  check('viewmodal_market_unitOfMeasure', 'Storage Unit of Measure value  is not valid').not().isEmpty().trim()
    .escape(),
  ...
  check('viewmodal_market_URL', 'Market URL value is not valid').not().isEmpty().trim()
    .escape(),
  ...
  check('viewmodal_storage_Description', 'Storage Description value is not valid').not().isEmpty().trim()
    .escape(),
...
(req, res) => {
  ...
  if (!result.isEmpty()) {
    ...
    const harvest_logid_uuid = req.body.viewmodal_harvest_logidSelect;
    const storage_logid_uuid = uuidv4();
    const storage_TimeStamp = moment(new Date(req.body.viewmodal_market_storageTimeStamp)).format('YYYY-MM-DD HH:mm:ss'); // actual time of storage/handover at market with farmer
    const storage_CaptureTime = moment(new Date()).format('YYYY-MM-DD HH:mm:ss'); // time of storage/handover data entry
    const logdatetime = moment(new Date()).format('YYYY-MM-DD HH:mm:ss');
    const lastmodifieddatetime = moment(new Date()).format('YYYY-MM-DD HH:mm:ss');

    // access req.body variables
    const req_harvest_suppliershortcode = req.body.viewmodal_harvest_suppliershortcode;
    const req_supplierproduce = req.body.viewmodal_supplierproduce;
    const req_market_Shortcode = req.body.viewmodal_market_Shortcode;
    const req_market_Name = req.body.viewmodal_market_Name;
    const req_market_Address = req.body.viewmodal_market_Address;
    const req_market_quantity = req.body.viewmodal_market_quantity;
    const req_market_unitOfMeasure = req.body.viewmodal_market_unitOfMeasure;
    const req_market_URL = req.body.viewmodal_market_URL;
    const req_storage_Description = req.body.viewmodal_storage_Description;
    const req_email = req.user.email;

    // blockchain variables
    const sys_storage_BlockchainHashID = '-';
    const sys_storage_BlockchainHashData = '-';
    const sys_storage_bool_added_to_blockchain = 'false';
    const sys_storage_added_to_blockchain_by = '-';
    const sys_storage_blockchain_uuid = '-';
    ...
    const sql = 'INSERT INTO foodprint_storage SET ?';
    try {
...
router.post('/update', [
  ...
  check('viewmodal_market_URL', 'Market URL value is not valid').not().isEmpty().trim()
    .escape(),
  check('viewmodal_storage_BlockchainHashID', 'Blockchain Hash ID value is not valid').not().isEmpty().trim()
    .escape(),
  check('viewmodal_storage_BlockchainHashData', 'Blockchain Hash Data value is not valid').not().isEmpty().trim()
    .escape(),
  check('viewmodal_storage_Description', 'Storage Description value is not valid').not().isEmpty().trim()
    .escape(),
  check('viewmodal_storage_bool_added_to_blockchain', 'Added to Blockchain value is not valid').not().isEmpty().trim()
    .escape(),
  // check('viewmodal_storage_added_to_blockchain_date', 'Storage Added to Blockchain Date is not valid').not().isEmpty(),
  check('viewmodal_storage_added_to_blockchain_by', 'Storage Added to Blockchain by is not valid').not().isEmpty().trim()
    .escape(),
  check('viewmodal_storage_blockchain_uuid', 'Storage Blockchain UUID value is not valid').not().isEmpty().trim()
    .escape(),
  check('viewmodal_storage_user', 'Sorage User  value is not valid').not().isEmpty().trim()
    .escape(),
  check('viewmodal_logdatetime', 'Logdatetime datetime value is not valid').not().isEmpty(),
  check('viewmodal_lastmodifieddatetime', 'Last Modified Datetime value is not valid').not(),
  check('viewmodal_harvest_logid', 'Harvest ID value is not valid').not().isEmpty().escape(),
  ...
  if (!result.isEmpty()) {
    ...
    console.log(`req.body.viewmodal_harvest_logid ${req.body.viewmodal_harvest_logid}`);
    const storage_TimeStamp = moment(new Date(req.body.viewmodal_market_storageTimeStamp)).format('YYYY-MM-DD HH:mm:ss');
    const storage_CaptureTime = moment(new Date(req.body.viewmodal_market_storageCaptureTime)).format('YYYY-MM-DD HH:mm:ss');
    const logdatetime = moment(new Date(req.body.viewmodal_logdatetime)).format('YYYY-MM-DD HH:mm:ss');
    const lastmodifieddatetime = moment(new Date(req.body.viewmodal_lastmodifieddatetime)).format('YYYY-MM-DD HH:mm:ss');
    // TODO - should rather update only the fields have changed!
    const sql = `UPDATE foodprint_storage SET harvest_logid='${req.body.viewmodal_harvest_logid}', `
...
router.post('/delete',
  [
    check('viewmodal_storage_logid', 'Storage ID value is not valid').not().isEmpty().trim()
      .escape(),
  ],
  (req, res) => {
    const sql = `DELETE FROM foodprint_storage WHERE storage_logid='${req.body.viewmodal_storage_logid}'`;
    // console.log('sql ' + sql);
    console.log(`configid ${req.body.viewmodal_storage_logid}`);
    if (req.user.role === ROLES.Admin || req.user.role === ROLES.Superuser) {
      const query = connection.execute(sql, (err, results) => {
        if (err) {
          // throw err;
          req.flash('error', err.message);
          // redirect to Storage Logbook page
