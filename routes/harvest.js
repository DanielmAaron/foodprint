...
router.get('/',
  ...
  (req, res, next) => {
    if (req.user.role === ROLES.Admin || req.user.role === ROLES.Superuser) {
      connection.execute('SELECT * FROM foodprint_harvest ORDER BY pk desc', (err, rows) => {
        if (err) {
          req.flash('error', err);
...
router.post('/save', upload.single('viewmodal_harvest_photohash_uploaded_file'),
  [
    ...
    check('viewmodal_harvest_suppliershortcode', 'Harvest Supplier Shortcode is not valid').not().isEmpty().trim()
      .escape(),
    check('viewmodal_harvest_suppliername', 'Harvest Supplier Name is not valid').not().isEmpty().trim()
      .escape(),
    check('viewmodal_harvest_supplieraddress', 'Harvest Supplier Address value is not valid').not().isEmpty().trim()
      .escape(),
    check('viewmodal_harvest_producename', 'Harvest Produce Name value is not valid').not().isEmpty().trim()
      .escape(),
    ...
    check('viewmodal_harvest_description', 'Harvest Description value is not valid').not().isEmpty().trim()
      .escape(),
    check('viewmodal_harvest_geolocation', 'Harvest GeoLocation value is not valid').not().isEmpty().trim()
      .escape(),
    check('viewmodal_harvest_quantity', 'Harvest Quantity value is not valid').not().isEmpty().trim()
      .escape(),
    check('viewmodal_harvest_unitofmeasure', 'Harvest Unit of Measure value is not valid').not().isEmpty().trim()
      .escape(),
  ...
  (req, res) => {
    ...
    if (!result.isEmpty()) {
      ...
      const harvest_logid_uuid = uuidv4();
      const harvest_TimeStamp = moment(new Date(req.body.viewmodal_harvest_timestamp)).format('YYYY-MM-DD HH:mm:ss'); // actual time of harvest in the field
      const harvest_CaptureTime = moment(new Date()).format('YYYY-MM-DD HH:mm:ss'); // time of harvest data entry
      ...
      const sql = 'INSERT INTO foodprint_harvest SET ?';
      try {
        fs.readFile(img.path, (err, img_datadata) => {
...
router.post('/update', upload.none(), [
  ...
  check('viewmodal_harvest_unitofmeasure', 'Harvest Unit of Measure value is not valid').not().isEmpty().trim()
    .escape(),
  // check('viewmodal_harvest_description_json', 'Harvest Description value is not valid').not().isEmpty().trim().escape(),
  check('viewmodal_harvest_blockchainhashid', 'Blockchain Hash ID value is not valid').not().isEmpty().trim()
    .escape(),
  check('viewmodal_harvest_blockchainhashdata', 'Blockchain Hash Data value is not valid').not().isEmpty().trim()
    .escape(),
  check('viewmodal_supplierproduce', 'Supplier Produce value is not valid').not().isEmpty().trim()
    .escape(),
  check('viewmodal_harvest_bool_added_to_blockchain', 'Added to Blockchain value is not valid').not().isEmpty().trim()
    .escape(),
  // check('viewmodal_harvest_added_to_blockchain_date', 'Harvest Added to Blockchain Date value is not valid').not().isEmpty(),
  check('viewmodal_harvest_added_to_blockchain_by', 'Harvest Added to Blockchain by value is not valid').not().isEmpty().trim()
    .escape(),
  check('viewmodal_harvest_blockchain_uuid', 'Harvest Blockchain UUID value is not valid').not().isEmpty().trim()
    .escape(),
  check('viewmodal_harvest_user', 'Harvest User value is not valid').not().isEmpty().trim()
    .escape(),
  check('viewmodal_logdatetime', 'Logdatetime datetime value is not valid').not().isEmpty(),
  check('viewmodal_lastmodifieddatetime', 'Last Modified Datetime value is not valid').not().isEmpty(),
  check('viewmodal_harvest_logid', 'Harvest ID value is not valid').not().isEmpty().trim()
    .escape(),
], (req, res) => {
  const result = validationResult(req);
  const { errors } = result;
  ...
  if (!result.isEmpty()) {
    ...
    console.log(`req.body.viewmodal_harvest_logid ${req.body.viewmodal_harvest_logid}`);
    const harvest_TimeStamp = moment(new Date(req.body.viewmodal_harvest_timestamp)).format('YYYY-MM-DD HH:mm:ss');
    const harvest_CaptureTime = moment(new Date(req.body.viewmodal_harvest_capturetime)).format('YYYY-MM-DD HH:mm:ss');
    ...
    const sql = `UPDATE foodprint_harvest SET harvest_supplierShortcode='${req.body.viewmodal_harvest_suppliershortcode}', `
                  + `harvest_supplierName='${req.body.viewmodal_harvest_suppliername
                  }',harvest_farmerName='${req.body.viewmodal_harvest_farmername
                  }',harvest_supplierAddress='${req.body.viewmodal_harvest_supplieraddress
                  }',year_established='${req.body.viewmodal_harvest_year_established
...
router.post('/delete', upload.none(),
  ...
  (req, res) => {
    const sql = `DELETE FROM foodprint_harvest WHERE harvest_logid='${req.body.viewmodal_harvest_logid}'`;
