...
contract('product', (accounts) => {
  ...
  it('should contain zero harvests in the beginning', async () => {
    // fetch instance of harvest contract
    const harvestInstance = await product.deployed();
    // get the number of harvests
    const harvestCounter = await harvestInstance.noHarvests();
    // check that there are no harvests initially
    assert.equal(harvestCounter, 0, 'initial number not equal to zero');
  });

  it('should add a harvest to the registry', async () => {
    const harvestInstance = await product.deployed();
    ...
    const harvest = await harvestInstance.harvestProduceArray(0);
