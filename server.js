...
const logger = require('morgan'); // Morgan is an HTTP request logger middleware for Node.js. It simplifies the process of logging requests to your application.
...
const db = require('./dbxml/localdb');
...
// food101
router.get('/food101', (req, res) => {
  res.render('food101', { user: req.user, page_name: 'food101' });
});

// tech101
router.get('/tech101', (req, res) => {
...
router.get('/scan/:id', (req, res) => {
  ...
  const boolTracedOnBlockchain = process.env.SHOW_TRACED_ON_BLOCKCHAIN || false;
...
router.get('/app/scan/:id', [sanitizeParam('id').escape().trim()],
  (req, res) => {
    const supplierProduceID = req.params.id; // OZCF_Apples or WMNP_Fennel
    const traceSqlBase = 'SELECT harvest_supplierShortcode, harvest_supplierName, harvest_farmerName,'
    ...
    connection.execute(traceSqlFinal,
      ...
      (err, rows) => {
        ...
        const marketID = CUSTOM_ENUMS.OZCF; // shortcode e.g. ozcf
        const logid = uuidv4();
        const qrid = ''; // TODO this is not yet being tracked in config

        // http://localhost:3000/app/scan/WMNP_Fennel
        // https://www.foodprintapp.com/app/scan/WMNP_Fennel
        const qrurl = `${req.protocol}://${req.get('host')}${req.originalUrl}`;

        const request_host = req.get('host');
        const request_origin = req.headers.referer;
        ...
        const request_useragent = req.headers['user-agent'];
...
router.get('/app/api/v1/scan/:id', [sanitizeParam('id').escape().trim()], (req, res) => {
  const supplierProduceID = req.params.id; // OZCF_Apples or WMNP_Fennel
  const traceSqlBase = 'SELECT harvest_supplierShortcode, harvest_supplierName, harvest_farmerName,'
        + 'year_established, harvest_description_json, harvest_photoHash, harvest_supplierAddress, harvest_produceName,'
        + 'harvest_TimeStamp, harvest_CaptureTime, harvest_Description, harvest_geolocation, supplierproduce,'
  ...
  connection.execute(traceSqlFinal,
    ...
    (err, rows) => {
      ...
      // START Track QR Scan (this could be done as xhr when scan page is rendered)
      const marketID = 'ozcf'; // shortcode e.g. ozcf
      const logid = uuidv4();
      const qrid = ''; // TODO this is  t yet being tracked in config
...
router.get('/checkin/:market_id', [sanitizeParam('market_id').escape().trim()], (req, res) => {
  const boolCheckinForm = process.env.SHOW_CHECKIN_FORM || false;
  const marketID = req.params.market_id; // shortcode e.g. ozcf
  const logid = uuidv4();
  const qrid = ''; // TODO this is not yet being tracked in config
  const qrurl = `${req.protocol}://${req.get('host')}${req.originalUrl}`;
  const request_host = req.get('host');
  ...
  // console.error('error', err)
    ...
    res.render('checkin.ejs', {
      data: marketID, showCheckinForm: boolCheckinForm, user: req.user, page_name: 'checkin',
    });
  }
  res.render('checkin.ejs', {
    data: marketID, showCheckinForm: boolCheckinForm, user: req.user, page_name: 'checkin',
...
router.post('/marketcheckin', [
  check('checkin_email', 'Your email is not valid').not().isEmpty().isEmail()
    .normalizeEmail(),
],
(req, res) => {
  ...
  if (!errors.isEmpty()) {
    ...
    const { checkin_market_id } = req.body;
    const { checkin_email } = req.body;
    const checkin_datetime = new Date();
    const checkin_firstname = '';
    const checkin_surname = '';
...
router.get('/test_db',
  ...
  async (req, res, next) => {
    if (req.user.role === ROLES.Admin || req.user.role === ROLES.Superuser) {
      try {
        connection.query('SELECT * FROM metaTable ORDER BY ProduceID desc', (err, rows) => {
          if (err) {
          // req.flash('error', err);
            console.error('error', err);
            res.render('./test_db', {
...
router.post('/app/addHarvest', (req, res) => {
...
router.post('/app/addStorage', (req, res) => {
...
router.post('/subscribe', [
  ...
  check('subscribe_email', 'Your email is not valid').not().isEmpty().isEmail()
    .normalizeEmail(),
],
(req, res) => {
  const errors = validationResult(req);
...
router.post('/app/testemail', (req, res) => {
  const mailOptions = {
...
router.post('/contactform', [
  check('contact_email', 'Contact email is not valid').not().isEmpty().isEmail()
    .normalizeEmail(),
...
(req, res) => {
  ...
  if (!errors.isEmpty()) {
    ...
    const { contact_fname } = req.body;
    const { contact_lname } = req.body;
    const { contact_message } = req.body;
    const contact_datetime = new Date();
    const contact_subject = 'FoodPrint Website Contact Enquiry';
    const contact_message_formatted = `<p>Email Sender: ${contact_email
...
router.post('/app/traceproduce', [
  check('search_term', 'Search term is not valid').not().isEmpty().trim()
    .escape(),
],
(req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    ...
    try {
      const storage_sql = `SELECT harvest_logid, storage_logid, market_storageTimeStamp, supplierproduce FROM foodprint_storage WHERE storage_logid='${req.body.search_term
      ...
      const harvest_sql = `SELECT harvest_logid, supplierproduce, harvest_quantity, harvest_unitofmeasure, harvest_TimeStamp  FROM foodprint_harvest WHERE harvest_logid='${req.body.search_term
      }' OR harvest_produceName='${req.body.search_term
      }' OR supplierproduce LIKE '%${req.body.search_term
      }%' OR supplierproduce='${req.body.search_term}'`;
      console.log(`harvest_sql ${harvest_sql}`);

      connection.execute(storage_sql, (err, storage_rows) => {
        if (err) {
          console.error('error', err);
          res.status.json({ err });
        } else {
          connection.execute(harvest_sql, (err, harvest_rows) => {
            if (err) {
              console.error('error', err);
              res.status.json({ err });
            } else {
...
router.post('/test_qrcode', async (req, res, next) => {
  try {
    ...
    const produceUrl = 'http://www.google.com';
    const supplier = 'supplier';
    const produce = 'Storage';
    const res2 = await QRCode.toDataURL(produceUrl);
    let QRFileName = supplier + produce;
    QRFileName = QRFileName.trim();
    const QRDirectory = '../static/';
    let QRFullName = `${QRDirectory + QRFileName}.png`;
